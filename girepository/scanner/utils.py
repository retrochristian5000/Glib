...
def get_libtool_command(options):
    ...
    try:
        subprocess.check_call([libtool_cmd, "--version"], stdout=open(os.devnull, "w"))
...
def which(program):
    ...
    def is_nt_exe(fpath):
        return (
            not fpath.lower().endswith(".exe")
            and os.path.isfile(fpath + ".exe")
            and os.access(fpath + ".exe", os.X_OK)
        )
