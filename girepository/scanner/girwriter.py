...
class GIRWriter(XMLWriter):
    ...
    def _write_generic(self, node):
        ...
        if hasattr(node, "version_doc") and node.version_doc:
            self.write_tag("doc-version", [("xml:space", "preserve")], node.version_doc)

        if hasattr(node, "deprecated_doc") and node.deprecated_doc:
            self.write_tag(
                "doc-deprecated", [("xml:space", "preserve")], node.deprecated_doc
            )

        if hasattr(node, "stability_doc") and node.stability_doc:
            self.write_tag(
                "doc-stability", [("xml:space", "preserve")], node.stability_doc
            )

        filepos = getattr(node, "get_main_position", lambda: None)()
        if filepos is not None:
            position = [
                ("filename", self._get_relative_path(filepos.filename)),
    ...
    def _write_function_macro(self, macro):
        attrs = [("name", macro.name), ("c:identifier", macro.symbol)]
        ...
        with self.tagcontext("parameters"):
            if callable.instance_parameter:
                self._write_parameter(
                    callable, callable.instance_parameter, "instance-parameter"
                )
    ...
    def _write_parameter(self, parent, parameter, nodename="parameter"):
        ...
        if (parameter.direction is not None) and (parameter.direction != "in"):
            ...
            attrs.append(
                ("caller-allocates", "1" if parameter.caller_allocates else "0")
            )
        if parameter.transfer:
            attrs.append(("transfer-ownership", parameter.transfer))
    ...
    def _write_type(self, ntype, relation=None, parent=None):
        ...
        if isinstance(ntype, girast.Varargs):
            ...
            elif ntype.zeroterminated and (
                ntype.size is not None or ntype.length_param_name is not None
            ):
    ...
    def _append_registered(self, node, attrs):
        ...
        if node.get_type:
            attrs.extend(
                [("glib:type-name", node.gtype_name), ("glib:get-type", node.get_type)]
            )
    ...
    def _write_class(self, node):
        ...
        if isinstance(node, girast.Class):
            ...
            if node.parent_type is not None:
                attrs.append(("parent", self._type_to_name(node.parent_type)))
            if node.is_abstract:
        ...
        if node.glib_type_struct is not None:
            attrs.append(
                ("glib:type-struct", self._type_to_name(node.glib_type_struct))
            )
        if isinstance(node, girast.Class):
            if node.fundamental:
        ...
        with self.tagcontext(tag_name, attrs):
            ...
            if isinstance(node, girast.Class):
                for iface in sorted(node.interfaces):
                    self.write_tag("implements", [("name", self._type_to_name(iface))])
            if isinstance(node, girast.Interface):
                for iface in sorted(node.prerequisites):
                    self.write_tag(
                        "prerequisite", [("name", self._type_to_name(iface))]
                    )
            if isinstance(node, girast.Class):
