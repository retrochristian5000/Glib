...
class GIRParser(object):
    ...
    def _parse_api(self, root):
        ...
        if version != COMPATIBLE_GIR_VERSION:
            raise SystemExit(
                "%s: Incompatible version %s (supported: %s)"
        ...
        self._namespace = girast.Namespace(
            ns.attrib["name"],
            ns.attrib["version"],
    ...
    def _parse_alias(self, node):
        ...
        alias = girast.Alias(
            node.attrib["name"], typeval, node.attrib.get(_cns("type"))
        )
    ...
    def _parse_generic_attribs(self, node, obj):
        ...
        if doc is not None:
            if doc.text:
                obj.doc = doc.text
                obj.doc_position = Position(
                    doc.attrib.get("filename", "<unknown>"),
        ...
        if hasattr(obj, "add_file_position"):
            positions = sorted(
                node.findall(_corens("source-position")),
                key=lambda x: (x.attrib["filename"], int(x.attrib["line"])),
            )
            for position in positions:
                ...
                obj.add_file_position(
                    Position(
                        position.attrib["filename"],
    ...
    def _parse_object_interface(self, node):
        ...
        for iface in self._find_children(node, _corens("prerequisite")):
            obj.prerequisites.append(
                self._namespace.type_from_name(iface.attrib["name"])
            )
    ...
    def _parse_function_common(self, node, klass, parent=None):
        ...
        if klass is girast.Callback:
            func = klass(
                name, retval, parameters, throws, node.attrib.get(_cns("type"))
            )
        ...
        parameters_node = node.find(_corens("parameters"))
        if parameters_node is not None:
            paramnode = self._find_first_child(
                parameters_node, _corens("instance-parameter")
            )
            if paramnode:
                func.instance_parameter = self._parse_parameter(paramnode)
            for paramnode in self._find_children(parameters_node, _corens("parameter")):
                parameters.append(self._parse_parameter(paramnode))
            for i, paramnode in enumerate(
                self._find_children(parameters_node, _corens("parameter"))
            ):
                param = parameters[i]
    ...
    def _parse_record(self, node, anonymous=False):
        ...
        if is_gtype_struct_for is not None:
            struct.is_gtype_struct_for = self._namespace.type_from_name(
                is_gtype_struct_for
            )
    ...
    def _parse_type_simple(self, typenode):
        ...
        # girast.Arrays have their own toplevel XML
            ...
            if name is None:
                ...
                subchild = self._find_first_child(
                    typenode,
                    list(map(_corens, ("callback", "array", "    varargs", "type"))),
                )
                ...
                return girast.Map(
                    subchildren_types[0], subchildren_types[1], ctype=ctype
                )
            else:
    ...
    def _parse_field(self, node, parent):
        ...
        if anonymous_elt is not None:
            if anonymous_elt.tag == _corens("callback"):
                anonymous_node = self._parse_function_common(
                    anonymous_elt, girast.Callback
                )
    ...
    def _parse_enumeration_bitfield(self, node):
        ...
        members = []
        obj = klass(
            name, ctype, members=members, gtype_name=type_name, get_type=get_type
        )
