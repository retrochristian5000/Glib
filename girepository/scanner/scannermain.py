...
def process_wrapper_args_begin(option, opt, value, parser, wrapper):
    ...
    if wrapper_cmd is None:
        parser.error(
            f"--{wrapper}-wrapper-args-begin must be called after --use-{wrapper}-wrapper"
        )
...
def get_cross_compile_option_group(parser):
    ...
    group.add_option(
        ...
        action="callback",
        callback=process_wrapper_binary,
    )
    group.add_option(
        "",
        "--binary-wrapper-args-begin",
        help="Start additional binary wrapper arguments",
        dest="wrapper",
        default=None,
        action="callback",
        callback=lambda option, opt, value, parser: process_wrapper_args_begin(
            option, opt, value, parser, "binary"
        ),
    )
    group.add_option(
        "",
        "--binary-wrapper-args-end",
        help="End additional binary wrapper arguments",
        action="callback",
        callback=process_wrapper_args_end,
    )
    ...
    group.add_option(
        ...
        action="callback",
        callback=lambda option, opt, value, parser: process_wrapper_args_begin(
            option, opt, value, parser, "ldd"
        ),
    )
    group.add_option(
        "",
        "--ldd-wrapper-args-end",
...
def get_preprocessor_option_group(parser):
    ...
    group.add_option(
        ...
        action="callback",
        callback=process_cflags_end,
    )
    group.add_option(
        "-I",
        help="Pre-processor include file",
        dest="cpp_includes",
        default=[],
        type="string",
        action="callback",
        callback=process_cpp_includes,
    )
    group.add_option(
        "-D",
...
def get_windows_option_group(parser):
    ...
    group.add_option(
        "-m",
        help="some machine dependent option",
        action="append",
        dest="m_option",
        default=[],
    )
...
def _get_option_parser():
    parser = optparse.OptionParser(
        "%prog [options] sources", version="%prog " + scanner.__version__
    )
    parser.add_option(
        "",
        "--quiet",
    ...
    parser.add_option(
        ...
        "--external-library",
        action="store_true",
        dest="external_library",
        default=False,
        help=(
            """If true, the library is located on the system,"""
    ...
    parser.add_option(
        ...
        dest="namespace_name",
        help=(
            "name of namespace for this unit, also "
...
the latter is not specified.""",
    ...
    parser.add_option(
        "",
        "--identifier-filter-cmd",
        action="store",
        dest="identifier_filter_cmd",
        default="",
...
match the namespace prefix.""",
    ...
    parser.add_option(
        ...
        dest="doc_format",
        help=(
            "name of the documentation format used in the project, "
...
def write_output(data, options):
    ...
    if options.output == "-":
        ...
        if os.path.isfile(options.output):
            ...
            os.chmod(
                main_f_name, stat.S_IWUSR | stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH
            )
...
def scanner_main(args):
    ...
    show_suppression = (
        options.warn_all is False
        and options.warn_strict is False
        and options.quiet is False
    )
